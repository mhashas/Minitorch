...
def test_other(x, y):
    ...
    """
    assert operators.relu(x) == max(0, x)
    assert operators.relu_back(x, y) == (y if x > 0 else 0.0)
...
def test_property(ls1, ls2):
    ...
    """
    assert_close(operators.add(operators.sum(ls1), operators.sum(ls2)), sum(ls1) + sum(ls2))
